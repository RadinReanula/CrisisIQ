...
/** Deterministic IDs so `seedDemoData` is idempotent (matches `supabase/seed.sql` needs body). */
const DEMO_NEED_UPSERT_ROWS: Omit<Need, 'created_at'>[] = [
  ...
  {
    id: 'n1000000-0000-4000-8000-000000000005',
  ...
  {
    id: 'n1000000-0000-4000-8000-000000000006',
  ...
  {
    id: 'n1000000-0000-4000-8000-000000000007',
    ...
    ai_matched_skills: ['rescue', 'driving'],
    status: 'assigned',
    assigned_volunteer_id: 'v1000000-0000-4000-8000-000000000002',
    event_id: '',
  },
  {
    id: 'n1000000-0000-4000-8000-000000000008',
    ...
    status: 'in_progress',
    assigned_volunteer_id: 'v1000000-0000-4000-8000-000000000004',
    event_id: '',
  },
  {
    id: 'n1000000-0000-4000-8000-000000000009',
    ...
    ai_matched_skills: ['cooking', 'logistics', 'driving'],
    status: 'assigned',
    assigned_volunteer_id: 'v1000000-0000-4000-8000-000000000003',
    event_id: '',
  },
  {
    ...
    ai_matched_skills: ['rescue', 'driving'],
    status: 'resolved',
    assigned_volunteer_id: 'v1000000-0000-4000-8000-000000000005',
    event_id: '',
  },
  {
    id: 'n1000000-0000-4000-8000-000000000011',
    ...
    ai_matched_skills: ['logistics', 'driving'],
    status: 'resolved',
    assigned_volunteer_id: 'v1000000-0000-4000-8000-000000000006',
    event_id: '',
  },
  {
    id: 'n1000000-0000-4000-8000-000000000012',
...
export function parseNeedRecord(value: unknown): Need | null {
  ...
  void needsBase; // readability guardrail
...
function parseVolunteerRecord(value: unknown): Volunteer | null {
  if (!isRecord(value)) {
    return null;
  }
...
function parseAssignmentRecord(value: unknown): Assignment | null {
  if (!isRecord(value)) {
    return null;
  }

  for (const key of ASSIGNMENT_ROW_KEYS) {
    if (!(key in value)) {
      // Optional keys may still be omitted on partial payloads — require core fields only:
      continue;
    }
  }
...
function parseAssignmentWithNeedRecord(value: unknown): (Assignment & { need: Need }) | null {
  if (!isRecord(value)) {
    return null;
  }

  const assignment = parseAssignmentRecord(value);
  const needCandidate = value['need'];
  const need = parseNeedRecord(needCandidate);
...
function parseNeedOrThrowUnknown(value: unknown): Need | null {
  return parseNeedRecord(value);
}
...
export async function submitNeed(need: SubmitNeedInput): Promise<Need | null> {
  try {
    ...
    return parseNeedOrThrowUnknown(data);
  } catch {
    return null;
  }
}
...
export async function getMyAssignments(
  ...
  try {
    ...
    for (const raw of data) {
      ...
      const rowWithNeedAlias = raw['need'];

      const mergedNeed = raw;
      mergedNeed.need = rowWithNeedAlias;

      const parsed = parseAssignmentWithNeedRecord(mergedNeed);

      // parseAssignmentWithNeedRecord expects flattened `need` key on the assignment row representation

      continue;
    }

    throw new Error('getMyAssignments: unexpected loop exit');
  } catch {
    return null;
  }
}
